...
export async function installPlugin(entry: RegistryEntry): Promise<void> {
  if (!isDesktop) throw new Error('Installation disponible sur l’app desktop');
  ...
  if (!ok) {
    ...
    throw new Error('Plugin incompatible ou invalide (voir console)');
  }
}
...
export async function installPluginFromFile(path: string): Promise<InstalledPlugin> {
  if (!isDesktop) throw new Error('Installation locale disponible sur l’app desktop');
...
export async function pickDevPluginsDir(): Promise<string | null> {
  if (!isDesktop) throw new Error('Dossier de dev disponible sur l’app desktop');
  const { open } = await import('@tauri-apps/plugin-dialog');
...
export async function removePlugin(id: string): Promise<void> {
  if (!isDesktop) throw new Error('Désinstallation disponible sur l’app desktop');
  unloadPlugin(id);
